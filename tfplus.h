...
public:
	...
	conv2d( const std::vector<int64>& filter,
		...
		m_padding(padding) { std::cout << "calling conv2d constructor!" << std::endl; }
	...
	node_output operator()(model& m, const node_output& input) const {
		...
		auto conv = Conv2D(m.root, input.output, w, strides_shape, "SAME");
...
struct softmax {
	node_output operator()(model& m, const node_output& input) const {
		m.tmp = Softmax(m.root, input.output);
		return node_output(m.tmp, input.shape);
	}
};
